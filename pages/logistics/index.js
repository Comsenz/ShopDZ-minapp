...
Page({
  ...
  onLoad: function (options) {
    ...
    // var refund_sn = '201904151746386403';
    self.not_used(order_sn);
  },
  //未使用
  not_used: function (order_sn){
    var self = this;
    var key = wx.getStorageSync('key');
    var order_sn = order_sn;
    wx.request({
      url: curl + 'api.php/Express/express_detail?order_sn=' + order_sn,
      method:'GET',
      success:function(res){
        if(res.data.code == 0){
          console.log(res);
          var list = res.data.data.express_detail;
